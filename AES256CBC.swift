...
final public class AES256CBC {
  ...
  public class func encryptString(_ str: String, key: String, iv: String) -> String? {
        if !str.isEmpty && key.characters.count == 32 {
...
final private class AESCipher {
    ...
    enum Variant: Int {
        ...
        var nkVariant: Int { // nkVariant words
            return [4, 6, 8][self.rawValue - 1]
        }

        var nbVariant: Int { // nbVariant words
            return 4
        }

        var nrVariant: Int { // nrVariant
            return nkVariant + 6
        }
    }
    ...
    private func encrypt(block: [UInt8]) -> [UInt8]? {
        let rounds = self.variant.nrVariant
        ...
        for r in 0..<rounds - 1 {
            ...
            let lb00 = tnull[Int(t[0] & 0xFF)]
            let lb01 = tone[Int((t[1] >> 8) & 0xFF)]
            let lb02 = ttwo[Int((t[2] >> 16) & 0xFF)]
            let lb03 = tthree[Int(t[3] >> 24)]
            b[0] = lb00 ^ lb01 ^ lb02 ^ lb03

            let lb10 = tnull[Int(t[1] & 0xFF)]
            let lb11 = tone[Int((t[2] >> 8) & 0xFF)]
            let lb12 = ttwo[Int((t[3] >> 16) & 0xFF)]
            let lb13 = tthree[Int(t[0] >> 24)]
            b[1] = lb10 ^ lb11 ^ lb12 ^ lb13

            let lb20 = tnull[Int(t[2] & 0xFF)]
            let lb21 = tone[Int((t[3] >> 8) & 0xFF)]
            let lb22 = ttwo[Int((t[0] >> 16) & 0xFF)]
            let lb23 = tthree[Int(t[1] >> 24)]
            b[2] = lb20 ^ lb21 ^ lb22 ^ lb23

            let lb30 = tnull[Int(t[3] & 0xFF)]
            let lb31 = tone[Int((t[0] >> 8) & 0xFF)]
            let lb32 = ttwo[Int((t[1] >> 16) & 0xFF)]
            let lb33 = tthree[Int(t[2] >> 24)]
    ...
    private func decrypt(block: [UInt8]) -> [UInt8]? {
        ...
        for r in (2...rounds).reversed() {
            ...
            let b00 = tnullINV[Int(t[0] & 0xFF)]
            let b01 = toneINV[Int((t[3] >> 8) & 0xFF)]
            let b02 = ttwoINV[Int((t[2] >> 16) & 0xFF)]
            let b03 = tthreeINV[Int(t[1] >> 24)]
            b[0] = b00 ^ b01 ^ b02 ^ b03

            let b10 = tnullINV[Int(t[1] & 0xFF)]
            let b11 = toneINV[Int((t[0] >> 8) & 0xFF)]
            let b12 = ttwoINV[Int((t[3] >> 16) & 0xFF)]
            let b13 = tthreeINV[Int(t[2] >> 24)]
            b[1] = b10 ^ b11 ^ b12 ^ b13

            let b20 = tnullINV[Int(t[2] & 0xFF)]
            let b21 = toneINV[Int((t[1] >> 8) & 0xFF)]
            let b22 = ttwoINV[Int((t[0] >> 16) & 0xFF)]
            let b23 = tthreeINV[Int(t[3] >> 24)]
            b[2] = b20 ^ b21 ^ b22 ^ b23

            let b30 = tnullINV[Int(t[3] & 0xFF)]
            let b31 = toneINV[Int((t[2] >> 8) & 0xFF)]
            let b32 = ttwoINV[Int((t[1] >> 16) & 0xFF)]
            let b33 = tthreeINV[Int(t[0] >> 24)]
            b[3] = b30 ^ b31 ^ b32 ^ b33
        }
    ...
    fileprivate func expandKeyInv(_ key: Key, variant: Variant) -> Array<Array<UInt32>> {
        ...
        for r in 1..<rounds {
            ...
            w = rk2[r][0]
            rk2[r][0] = uOne[Int(B0(w))] ^ uTwo[Int(B1(w))] ^ uThree[Int(B2(w))] ^ uFour[Int(B3(w))]

            w = rk2[r][1]
            rk2[r][1] = uOne[Int(B0(w))] ^ uTwo[Int(B1(w))] ^ uThree[Int(B2(w))] ^ uFour[Int(B3(w))]

            w = rk2[r][2]
            rk2[r][2] = uOne[Int(B0(w))] ^ uTwo[Int(B1(w))] ^ uThree[Int(B2(w))] ^ uFour[Int(B3(w))]

            w = rk2[r][3]
            rk2[r][3] = uOne[Int(B0(w))] ^ uTwo[Int(B1(w))] ^ uThree[Int(B2(w))] ^ uFour[Int(B3(w))]
        }
    ...
    fileprivate func expandKey(_ key: Key, variant: Variant) -> Array<Array<UInt32>> {
        ...
        var w = Array<UInt8>(repeating: 0, count: variant.nbVariant * (variant.nrVariant + 1) * 4)
        for i in 0..<variant.nkVariant {
            for wordIdx in 0..<4 {
        ...
        for i in variant.nkVariant..<variant.nbVariant * (variant.nrVariant + 1) {
            tmp = Array<UInt8>(repeating: 0, count: 4)

            for wordIdx in 0..<4 {
                tmp[wordIdx] = w[4*(i-1)+wordIdx]
            }
            if (i % variant.nkVariant) == 0 {
                tmp = subWord(rotateLeft(UInt32(bytes: tmp), by: 8).bytes(totalBytes: MemoryLayout<UInt32>.size))
                tmp[0] = tmp.first! ^ rcon[i/variant.nkVariant]
            } else if variant.nkVariant > 6 && (i % variant.nkVariant) == 4 {
                tmp = subWord(tmp)
            }

            // xor array of bytes
            for wordIdx in 0..<4 {
                w[4*i+wordIdx] = w[4*(i-variant.nkVariant)+wordIdx]^tmp[wordIdx]
            }
        }
...
extension AESCipher {
    ...
    fileprivate func B0(_ number: UInt32) -> UInt32 {
        return number & 0xFF
    }

    fileprivate func B1(_ number: UInt32) -> UInt32 {
        return (number >> 8) & 0xFF
    }

    fileprivate func B2(_ number: UInt32) -> UInt32 {
        return (number >> 16) & 0xFF
    }

    fileprivate func B3(_ number: UInt32) -> UInt32 {
        return (number >> 24) & 0xFF
    }

    fileprivate func F1(_ x0: UInt32, _ x1: UInt32, _ x2: UInt32, _ x3: UInt32) -> UInt32 {
        var result: UInt32 = 0
        result |= UInt32(B1(tnull[Int(x0 & 255)]))
        result |= UInt32(B1(tnull[Int((x1 >> 8) & 255)])) << 8
        result |= UInt32(B1(tnull[Int((x2 >> 16) & 255)])) << 16
        result |= UInt32(B1(tnull[Int(x3 >> 24)])) << 24
        return result
    }
...
fileprivate func xor(_ numArrayA: Array<UInt8>, _ numArrayB: Array<UInt8>) -> Array<UInt8> {
    var xored = Array<UInt8>(repeating: 0, count: min(numArrayA.count, numArrayB.count))
    for i in 0..<xored.count {
        xored[i] = numArrayA[i] ^ numArrayB[i]
    }
    return xored
}
